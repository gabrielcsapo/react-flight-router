...
`getRequest()` is available automatically in server components and actions — no `onRequest` callback needed for basic request context. See the [Request Context guide](../guides/request-context.md) for details.
