...
export default async function PostsIndexPage() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-4">Fetched {posts.length} posts at {new Date().toISOString()}</p>
      <ul className="space-y-4">
        {posts.map((post) => (
          <li key={post.id} className="border border-gray-200 rounded-lg p-5 bg-white">
            ...
            <Link to={`/users/${post.userId}`} className="text-xs text-gray-400 hover:text-gray-600">
              View Author
            </Link>
          </li>
        ))}
